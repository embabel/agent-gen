...
Demonstrates framework-agnostic `DefaultToolLoop` with different LLM backends.
...
| `LangChainToolLoopIntegrationTest` | LangChain4j backend proving framework independence                                                                  | Same callbacks work with LangChain4j; tool invocations tracked                |
...
```bash
mvn test -Dtest=DefaultToolLoopIntegrationTest
mvn test  -Dtest=ParallelToolLoopIntegrationTest
mvn test  -Dtest=LangChainToolLoopIntegrationTest
mvn test  -Dtest=OpenAiToolLoopIntegrationTest
```
